...
def test_login_ok(client, seed_uy_currency):
    _register(client)
...
def test_login_wrong_password(client, seed_uy_currency):
    _register(client)
...
def test_login_unknown_email(client, seed_uy_currency):
    resp = client.post("/auth/login", json={"email": "nadie@b.com", "password": "12345678"})
    assert resp.status_code == 401
    assert resp.json()["code"] == "credentials_invalid"


def test_login_soft_deleted_user(client, db_session, seed_uy_currency):
    _register(client)
